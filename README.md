...
삭제 여부의 자동 판정과 첨부파일 다운로드는 아직 포함하지 않습니다. 공고가 최신
페이지에서 보이지 않는다는 이유만으로 삭제됐다고 판단할 수 없기 때문입니다.
...
robots.txt를 확인하고, 요청 간격을 지나치게 줄이지 마세요.
...
테스트는 저장된 HTML fixture만 사용하므로 사이트에 접속하지 않습니다.
...
`data/`의 SQLite 파일은 Git에 올리지 않도록 설정되어 있습니다. 주기적으로 프로그램을
종료한 상태에서 파일을 `data/backups/` 등 다른 위치에 복사해 두는 것이 좋습니다.
